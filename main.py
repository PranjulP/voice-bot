...
async def send_tts_chunk(websocket: WebSocket, text: str, t_before: float, already_logged: bool, t_start: float = None):
    ...
    response = httpx.post(
        "https://api.deepgram.com/v1/speak?model=aura-2-en&container=mp3",
        headers={
    ...
    print(f"Deepgram status: {response.status_code}, content-type: {response.headers.get('content-type')}, bytes: {len(audio)}")
